...
(function () {
  ...
  function withinView(el) {
    const bodyHeight = Math.min(document.body.clientHeight, document.documentElement.clientHeight);
  ...
  function fire(element) {
    ...
    if (style2) {
      element.setAttribute("style", element.getAttribute("style") + "; " + style2);
    }
  }
  ...
  function play() {
    ...
    getEmergeElements().forEach(function (self, index, emerging) {
      if (self.dataset.await && document.getElementById(self.dataset.await) === null) {
      ...
      if (effect) {
        ...
        if (fxData) {
          style1 += `${cssTransform}: ${fxData.one};` + `${cssTransformOrigin}: ${fxData.orn};`;
          style2 +=
            cssTransform +
            ": " +
            fxData.two +
      ...
      first.push(
        Promise.all(
          [self].concat(Array.from(self.querySelectorAll("*"))).reduce(function (sources, element) {
            if (element.nodeName.toLowerCase() === "img") {
              sources.push(imgLoaded(element.src));
            } else if (element.nodeName.toLowerCase() === "video") {
              sources.push(
                eventDispatched(element, element.dataset["emerge-event"] || "canplaythrough")
              );
            }
      ...
      if (self.dataset.spin) {
        ...
        if (customSpinner !== null) {
          ...
          const spinnerOptions = Object.keys(spinner_defaults).reduce(function (options, key) {
            options[key] =
              self.dataset[key] === undefined ? spinner_defaults[key] : self.dataset[key];
            return options;
          }, {});
  ...
  if (window.IntersectionObserver === undefined || document.documentElement.animate === undefined) {
    return;
  }
